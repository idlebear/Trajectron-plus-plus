...
def get_rotation_matrix2d(center: torch.Tensor, angle: torch.Tensor, scale: torch.Tensor) -> torch.Tensor:
    ...
    M[..., 0, 2] = (torch.tensor(1.0, device=center.device) - alpha) * x - beta * y
    M[..., 1, 2] = beta * x + (torch.tensor(1.0, device=center.device) - alpha) * y
    return M
...
def homography_warp(
    ...
    warper = HomographyWarper(x_min, y_min, x_max, y_max, out_height, out_width, mode, padding_mode, device=centers.device)
...
class HomographyWarper(nn.Module):
    ...
    def __init__(
        ...
        self.grid: torch.Tensor = create_batched_meshgrid(x_min, y_min, x_max, y_max, height, width, self.device)
